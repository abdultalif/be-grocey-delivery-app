...
export class ProvincesSeeder {
  public async run(dataSource: DataSource): Promise<void> {
    const provincesRepository = dataSource.getRepository(Provinces);

    const existingProvinces = await provincesRepository.find();
    ...
    await provincesRepository.save(provinces);
  }
}
